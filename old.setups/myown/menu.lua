...
function menu:item_enter(num, opts)
    ...
    local item_content = item._background:get_children()[1]:get_children()
    if item_content[2] and "wibox.widget.imagebox" == tostring(item_content[2]):match("[a-z.]+") then
	    local bg = item_content[2]
	    local image = rawget(bg, "_private").image
	    local theme = item.theme
	    bg:set_image(gears.color.recolor_image(image, theme.fg_focus))
    end
...
function menu:item_leave(num)
	...
	if item_content[2] and "wibox.widget.imagebox" == tostring(item_content[2]):match("[a-z.]+") then
		...
		local image = rawget(bg, "_private").image
		local theme = item.theme
		bg:set_image(gears.color.recolor_image(image, theme.fg_normal))
	end
end
...
function menu:add(args, index)
    ...
    -- write it like this
    -- to make get_children_by_id available
    item._background = wibox.widget{
	    widget = wibox.container.background,
	    item.widget,
	    fg = item.theme.fg_normal,
	    bg = item.theme.bg_normal,
    }
